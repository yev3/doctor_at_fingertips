...
ExampleDebugStatEntry_t xIPTraceValues[] =
  {
    ...
    {iptraceID_NETWORK_INTERFACE_RECEIVE,
     "Packets received by the network interface", prvIncrementEventCount, 0},
    {iptraceID_NETWORK_INTERFACE_TRANSMIT, "Count of transmitted packets",
     prvIncrementEventCount, 0},
    {iptraceID_PACKET_DROPPED_TO_GENERATE_ARP,
     "Count of packets dropped to generate ARP", prvIncrementEventCount, 0},
    {iptraceID_NETWORK_BUFFER_OBTAINED, "Lowest ever available network buffers",
     prvStoreLowest, 0xffffUL},
    {iptraceID_NETWORK_EVENT_RECEIVED,
     "Lowest ever free space in network event queue", prvStoreLowest, 0xffffUL},
    {iptraceID_FAILED_TO_OBTAIN_NETWORK_BUFFER,
     "Count of failed attempts to obtain a network buffer",
     prvIncrementEventCount, 0},
    {iptraceID_ARP_TABLE_ENTRY_EXPIRED, "Count of expired ARP entries",
     prvIncrementEventCount, 0},
    {iptraceID_FAILED_TO_CREATE_SOCKET, "Count of failures to create a socket",
     prvIncrementEventCount, 0},
    {iptraceID_RECVFROM_DISCARDING_BYTES,
     "Count of times recvfrom() has discarding bytes", prvIncrementEventCount,
     0},
    {iptraceID_ETHERNET_RX_EVENT_LOST,
     "Count of lost Etheret Rx events (event queue full?)",
     prvIncrementEventCount, 0},
    {iptraceID_STACK_TX_EVENT_LOST,
     "Count of lost IP stack events (event queue full?)",
     prvIncrementEventCount, 0},
    {ipconfigID_BIND_FAILED, "Count of failed calls to bind()",
     prvIncrementEventCount, 0},
    {iptraceID_RECVFROM_TIMEOUT, "Count of receive timeouts",
     prvIncrementEventCount, 0},
    {iptraceID_SENDTO_DATA_TOO_LONG,
     "Count of failed sends due to oversized payload", prvIncrementEventCount,
     0},
    {iptraceID_SENDTO_SOCKET_NOT_BOUND,
     "Count of failed sends due to unbound socket", prvIncrementEventCount, 0},
    {iptraceID_NO_BUFFER_FOR_SENDTO, "Count of failed transmits due to timeout",
     prvIncrementEventCount, 0},
    {iptraceID_WAIT_FOR_TX_DMA_DESCRIPTOR,
     "Number of times task had to wait to obtain a DMA Tx descriptor",
     prvIncrementEventCount, 0},
    {iptraceID_FAILED_TO_NOTIFY_SELECT_GROUP, "Failed to notify select group",
     prvIncrementEventCount, 0},
    {iptraceID_TOTAL_NETWORK_BUFFERS_OBTAINED, "Total network buffers obtained",
     prvIncrementEventCount, 0},
    {iptraceID_TOTAL_NETWORK_BUFFERS_RELEASED, "Total network buffers released",
     prvIncrementEventCount, 0}
...
/**
 * Status board on the LCD
 */
void formatLCDStats() {
  //                        (0, 5)      (0, 17)
  lcd_printf_at(0, 0, "rcvd ____ / sent ____");
  lcd_printf_at(1, 0, "bfNew ____ bfDel ____");
  lcd_printf_at(2, 0, "sndTO ____ rcvTO ____");
  lcd_printf_at(3, 0, "bfLow ____ evtLw ____");
  lcd_printf_at(4, 0, "# dropped genARP ____");
  lcd_printf_at(5, 0, "cnt bfalloc fail ____");
  lcd_printf_at(6, 0, "cnt sock cr fail ____");
  lcd_printf_at(7, 0, "cnt recvfm discB ____");
  lcd_printf_at(8, 0, "lost Eth RX evt  ____");
  lcd_printf_at(9, 0, "lost IPStk evt   ____");
  lcd_printf_at(10, 0, "cnt failed bind()____");
  lcd_printf_at(11, 0, "cnt send unbound ____");
//	lcd_printf_at(11, 0, "cnt send too big ____");
...
BaseType_t xExampleDebugStatEntries(void) {
  ...
  return (BaseType_t) (sizeof(xIPTraceValues) /
                       sizeof(ExampleDebugStatEntry_t));
}

/**
 * Update the counts on the lcd
 */
void lcdCntUpdate(uint8_t id, uint32_t cnt) {
  int row = -1;
  int col = -1;
  switch (id) {
    case iptraceID_NETWORK_INTERFACE_RECEIVE: row = 0;
      col = 5;
      break;
    case iptraceID_NETWORK_INTERFACE_TRANSMIT: row = 0;
      col = 17;
      break;
    case iptraceID_PACKET_DROPPED_TO_GENERATE_ARP: row = 4;
      col = 17;
      break;
    case iptraceID_NETWORK_BUFFER_OBTAINED: row = 3;
      col = 6;
      break;
    case iptraceID_NETWORK_EVENT_RECEIVED: row = 3;
      col = 17;
      break;
    case iptraceID_FAILED_TO_OBTAIN_NETWORK_BUFFER: row = 5;
      col = 17;
      break;
    case iptraceID_ARP_TABLE_ENTRY_EXPIRED: row = 0;
      col = 17;
      break;
    case iptraceID_FAILED_TO_CREATE_SOCKET: row = 6;
      col = 17;
      break;
    case iptraceID_RECVFROM_DISCARDING_BYTES: row = 7;
      col = 17;
      break;
    case iptraceID_ETHERNET_RX_EVENT_LOST: row = 8;
      col = 17;
      break;
    case iptraceID_STACK_TX_EVENT_LOST: row = 9;
      col = 17;
      break;
    case ipconfigID_BIND_FAILED: row = 10;
      col = 17;
      break;
    case iptraceID_RECVFROM_TIMEOUT: row = 2;
      col = 17;
      break;
    case iptraceID_SENDTO_DATA_TOO_LONG: break;
    case iptraceID_SENDTO_SOCKET_NOT_BOUND: row = 11;
      col = 17;
      break;
    case iptraceID_NO_BUFFER_FOR_SENDTO: row = 2;
      col = 6;
      break;
    case iptraceID_WAIT_FOR_TX_DMA_DESCRIPTOR: break;
    case iptraceID_FAILED_TO_NOTIFY_SELECT_GROUP: break;
    case iptraceID_TOTAL_NETWORK_BUFFERS_OBTAINED: row = 1;
      col = 6;
      break;
    case iptraceID_TOTAL_NETWORK_BUFFERS_RELEASED: row = 1;
      col = 17;
      break;
    default:break;
  }
...
void vExampleDebugStatUpdate(uint8_t ucIdentifier, uint32_t ulValue) {
  static uint8_t lcdInitialized = 0;
  if (!lcdInitialized) {
    lcdInitialized = 1;
    lcd_init();
    formatLCDStats();
  ...
  BaseType_t xIndex;
  const BaseType_t
    xEntries = sizeof(xIPTraceValues) / sizeof(ExampleDebugStatEntry_t);
  ...
  for (xIndex = 0; xIndex < xEntries; xIndex++) {
    if (xIPTraceValues[xIndex].ucIdentifier == ucIdentifier) {
      xIPTraceValues[xIndex].vPerformAction(&(xIPTraceValues[xIndex].ulData),
                                            ulValue);
      lcdCntUpdate(ucIdentifier, xIPTraceValues[xIndex].ulData);
      break;
    }
  }
